...
2. **Reviewing and Approving/Rejecting Food Items:**
   ...
   - Ingredients are color-coded:
     - **Green**: Approved ingredients, present in the checklist database which shows (Approved).
     - **Amber**: Not in the checklist database, but generally considered safe which shows (Not Approved).
     - **Red**: Present in the ingredients.json for unsafe ingredients which shows (Not Safe).
   - The manager can approve or reject the item based on the ingredient list, colour highlighted and the AI-driven advisories.
...
- **Compliance Verification:** Conduct compliance checks using checklists database for approved and not approved ingredients and ingredients.json for unsafe ingredients.
...
- **Compliance Checks:**
  - Use integrated OCR and Machine Learning tools, such as Tesseract.js and OpenAI API, to verify label information.
...
3. **Conducting Compliance Checks:**
   ...
   - Approve or reject ingredients by editing and based on the compliance analysis.
...
## Glossary
  ...
  - **AI-driven analysis**: The use of artificial intelligence OpenAI, to evaluate and make decisions based on data. In this application, AI is used to analyze food labels and check for compliance with health and safety standards.

  - **OCR (Optical Character Recognition)**: Technology that recognizes text within a digital image. It is used in this system called Tesseract.js to extract text from uploaded food label images for further analysis and compliance checking.
